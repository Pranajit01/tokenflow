/**
 * TokenSuccessPage.jsx — Token Success Page Layout Fix
 */

import { useLocation, Link, Navigate } from 'react-router';
import { ListOrdered, Home, PartyPopper } from 'lucide-react';
    ...
    <div className="min-h-screen w-full py-12 md:py-20 px-4 md:px-8 relative flex flex-col justify-center items-center overflow-hidden">
      ...
      <div className="max-w-xl w-full mx-auto relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-extrabold mb-2 tracking-tight break-words">
            Token Generated Successfully!
          </h1>
          <p className="text-sm text-white/60 leading-relaxed break-words">Your digital queue token is now live in the system</p>
        </div>

        {/* Token Display Card */}
        <div className="space-card p-6 sm:p-8 text-center mb-6 border border-[#12b3a4]/40 shadow-[0_0_30px_rgba(18,179,164,0.2)]">
          <p className="text-xs font-mono uppercase text-white/50 mb-2 tracking-wider">Assigned Digital Token ID</p>
          <h2 className="text-4xl sm:text-5xl font-extrabold tracking-widest text-[#12b3a4] font-mono break-all">
            {token.tokenId}
          </h2>
        </div>
          ...
          <div className="space-card p-5 mt-6 border border-white/15">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="min-w-0">
                ...
                <p className="font-semibold text-white break-words">{aiAnalysis.service}</p>
              </div>
              <div className="min-w-0">
                <span className="text-white/40 block mb-1">Target Department:</span>
                <p className="font-semibold text-white break-words">{aiAnalysis.department}</p>
              </div>
              <div className="min-w-0">
                <span className="text-white/40 block mb-1">Priority Level:</span>
                <p className="font-semibold text-white capitalize break-words">{aiAnalysis.priority?.level?.replace('_', ' ')}</p>
              </div>
              <div className="min-w-0">
                <span className="text-white/40 block mb-1">Appointment:</span>
                <p className="font-semibold text-white break-words">{aiAnalysis.isAppointment ? 'Yes (Confirmed)' : 'No (Walk-in)'}</p>
              </div>
            </div>
            {aiAnalysis.priority?.reason && (
              <p className="text-xs text-white/50 mt-3 pt-3 border-t border-white/10 italic break-words">
        ...
        <div className="flex flex-wrap gap-4 mt-8 justify-center w-full">
          <Link to="/live" className="btn-primary flex-1 sm:flex-initial justify-center">
            <ListOrdered size={16} />
            <span>Track Live Queue Stream</span>
          </Link>
          <Link to="/" className="btn-outline flex-1 sm:flex-initial justify-center">
            <Home size={16} />
